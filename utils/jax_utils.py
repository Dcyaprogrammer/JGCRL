...
import os
import pickle
from typing import Any, Dict, Mapping, Sequence,NamedTuple
...
class RMS(flax.struct.PyTreeNode):
    """Running mean and std that can be used in jax.lax.scan"""
    M: jnp.ndarray
    S: jnp.ndarray 
    n: float

    @classmethod
    def create(cls, shape=(1,), epsilon=1e-4):
        return cls(
            M=jnp.zeros(shape),
            S=jnp.ones(shape),
            n=epsilon
    ...
    def __call__(self, x):
        ...
        delta = jnp.mean(x, axis=(0,1)) - self.M
        new_M = self.M + delta * bs / (self.n + bs)
        new_S = (self.S * self.n + jnp.var(x, axis=(0, 1)) * bs + 
